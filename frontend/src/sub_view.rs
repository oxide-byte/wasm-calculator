...
const WASM_BYTES: &[u8] = include_bytes!("../../modules/subber.wasm");
...
impl WasmSubModule {
    ...
    pub fn subber(&self, a: i32, b: i32) -> leptos::error::Result<i32, JsValue> {
        ...
        // Try to get the subber function
        let subber_fn = match js_sys::Reflect::get(&exports, &JsValue::from_str("sub")) {
            Ok(func) => {
        ...
        match subber_fn.call2(&JsValue::NULL, &JsValue::from(a), &JsValue::from(b)) {
            Ok(result) => {
                match result.as_f64() {
                    Some(num) => {
                        Ok(num as i32)
            ...
            (Ok(n1), Ok(n2)) => {
                ...
                spawn_local(async move {
                    match WasmSubModule::new() {
                        Ok(wasm_module) => {
                            match wasm_module.subber(n1, n2) {
                                Ok(sum) => {
                                    result.set(Some(sum));
                                    error.set(None);
                                },
