...
const WASM_BYTES: &[u8] = include_bytes!("../../../modules/power.wasm");

#[wasm_bindgen]
pub struct WasmPowerModule {
    instance: js_sys::WebAssembly::Instance,
}

#[wasm_bindgen]
impl WasmPowerModule {
    #[wasm_bindgen(constructor)]
    pub fn new() -> leptos::error::Result<WasmPowerModule, JsValue> {
        // Create module from bytes
        info!("WASM [power] construct...");
        ...
        info!("WASM [power] instance created");
        Ok(WasmPowerModule { instance })
    }

    pub fn power(&self, a: i32, b: i32) -> leptos::error::Result<i32, JsValue> {
        info!("CALL [power] for {} {}", a, b);
        ...
        // Try to get the power function Look WAT
        let power_fn = match js_sys::Reflect::get(&exports, &JsValue::from_str("c")) {
            Ok(func) => {
                info!("Found power function: {:?}", func);
                match func.dyn_into::<js_sys::Function>() {
                    Ok(f) => f,
                    Err(e) => {
                        error!("Failed to convert to Function: {:?}", e);
                        return Err(JsValue::from_str("power is not a function"));
                    }
                }
            },
            Err(e) => {
                error!("Failed to get power function: {:?}", e);
                return Err(e);
            }
        };

        // Call the function with better error handling
        match power_fn.call2(&JsValue::NULL, &JsValue::from(a), &JsValue::from(b)) {
            Ok(result) => {
                match result.as_f64() {
                    Some(num) => {
                        Ok(num as i32)
            ...
            Err(e) => {
                error!("Error calling power function: {:?}", e);
                Err(e)
            }
        }
    }
...
#[component]
pub fn PowerView(v1: RwSignal<String>, v2: RwSignal<String>) -> impl IntoView {
        ...
        match (n1_str.parse::<i32>(), n2_str.parse::<i32>()) {
            (Ok(n1), Ok(n2)) => {
                ...
                spawn_local(async move {
                    match WasmPowerModule::new() {
                        Ok(wasm_module) => {
                            match wasm_module.power(n1, n2) {
                                Ok(sum) => {
                                    result.set(Some(sum));
                                    error.set(None);
                                },
    ...
    view! {
        <div class="max-w-md mx-auto mt-10 mt-3 p-5 bg-white rounded-lg shadow-lg">
            <p class="mb-5"> POWER calculated with a Python WASM module </p>
