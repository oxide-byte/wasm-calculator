...
const WASM_BYTES: &[u8] = include_bytes!("../../../modules/digit-count.wasm");

#[wasm_bindgen]
pub struct WasmDigitCountModule {
    instance: js_sys::WebAssembly::Instance,
}

#[wasm_bindgen]
impl WasmDigitCountModule {
    #[wasm_bindgen(constructor)]
    pub fn new() -> leptos::error::Result<WasmDigitCountModule, JsValue> {
        // Create module from bytes
        info!("WASM [digit_count] construct...");
        ...
        info!("WASM [digit_count] instance created");
        Ok(WasmDigitCountModule { instance })
    }

    pub fn digit_count(&self, a: i32, b: i32) -> leptos::error::Result<i32, JsValue> {
        info!("CALL [digit_count] for {} {}", a, b);
        ...
        // Try to get the digit_count function
        let digit_count_fn = match js_sys::Reflect::get(&exports, &JsValue::from_str("countDigits")) {
            Ok(func) => {
                info!("Found digit_count function: {:?}", func);
                match func.dyn_into::<js_sys::Function>() {
                    Ok(f) => f,
                    Err(e) => {
                        error!("Failed to convert to Function: {:?}", e);
                        return Err(JsValue::from_str("digit_count is not a function"));
                    }
                }
            },
            Err(e) => {
                error!("Failed to get digit_count function: {:?}", e);
                return Err(e);
            }
        };

        // Call the function with better error handling
        match digit_count_fn.call2(&JsValue::NULL, &JsValue::from(a), &JsValue::from(b)) {
            Ok(result) => {
                match result.as_f64() {
                    Some(num) => {
                        Ok(num as i32)
            ...
            Err(e) => {
                error!("Error calling digit_count function: {:?}", e);
                Err(e)
            }
        }
    }
...
#[component]
pub fn DigitCountView(v1: RwSignal<String>, v2: RwSignal<String>) -> impl IntoView {
        ...
        match (n1_str.parse::<i32>(), n2_str.parse::<i32>()) {
            (Ok(n1), Ok(n2)) => {
                ...
                spawn_local(async move {
                    match WasmDigitCountModule::new() {
                        Ok(wasm_module) => {
                            match wasm_module.digit_count(n1, n2) {
                                Ok(sum) => {
                                    result.set(Some(sum));
                                    error.set(None);
                                },
    ...
    view! {
        <div class="max-w-md mx-auto mt-10 mt-3 p-5 bg-white rounded-lg shadow-lg">
            <p class="mb-5"> Digit Counter calculated with a ZIG WASM module </p>
